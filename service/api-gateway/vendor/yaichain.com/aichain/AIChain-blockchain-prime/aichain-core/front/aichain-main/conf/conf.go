...
import (
	...
	"io/ioutil"
	"time"
	"yaichain.com/aichain/AIChain-blockchain-prime/aichain-common/iniconfig"
)
...
type Redis struct {
	...
	DefaultKey  string `ini:"aichain_key"`
}

type Nsq struct {
	HttpUrl           string `ini:"http_url"`
	Tcp               string `json:"tcp"`
	TopicFileName     string `ini:"topic_file_name"`
	TopicTransferName string `ini:"topic_transfer_name"`
}

type Nginx struct {
	Ip              string `ini:"ip"`
	IpfsPort        string `ini:"ipfs_port"`
	IpfsClusterPort string `ini:"ipfs_cluster_port"`
}
...
func readConfig() (*CommonConfig, error) {
	data, err := ioutil.ReadFile("./conf/aichain-config.ini")
	if err != nil {
		fmt.Println(err)
	...
	case DevMode:
		return readConfigByMode("./conf/aichain-env-dev.ini", DevMode)
	case TestMode:
		return readConfigByMode("./conf/aichain-env-test.ini", TestMode)
	case ProdMode:
		return readConfigByMode("./conf/aichain-env-prod.ini", ProdMode)
	}
